...
std::vector<int> getValidMoves(PuzzleState state) {
    ...
    if (state.emptyRow > 0) validMoves.push_back(0); // cima
    if (state.emptyRow < 2) validMoves.push_back(1); // baixo
    if (state.emptyCol > 0) validMoves.push_back(2); // esquerda
    if (state.emptyCol < 2) validMoves.push_back(3); // direita
